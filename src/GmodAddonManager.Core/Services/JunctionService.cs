...
namespace GmodAddonManager.Core.Services
{
    public class JunctionService : IJunctionService
    {
                        ...
                        try
                        {
                            ...
                            else
                            {
                                ...
                                Task.Run(async () =>
                                {
                                    ...
                                    {
                                        Debug.WriteLine($"[JunctionService] Failed to cleanup backup directory {backupPath}: {cleanupEx.Message}");
                                    }
                                });
                            }
                        }
            ...
            finally
            {
                try
                {
                    if (Directory.Exists(junctionDir))
                    {
                        try
                        {
                            RemoveJunction(junctionDir);
                        }
                        catch (Exception removeEx)
                        {
                            Debug.WriteLine($"[JunctionService] Failed to remove test junction {junctionDir}: {removeEx.Message}");
                        }

                        try
                        {
                            Directory.Delete(junctionDir, true);
                        }
                        catch (Exception deleteEx)
                        {
                            Debug.WriteLine($"[JunctionService] Failed to delete test junction directory {junctionDir}: {deleteEx.Message}");
                        }
                    }
        ...
        public bool IsHardLink(string filePath, string targetPath)
        {
            ...
            if (TryGetFileId(filePath, out var fileId1) && TryGetFileId(targetPath, out var fileId2))
            {
                return fileId1.VolumeSerialNumber == fileId2.VolumeSerialNumber &&
                       fileId1.FileIndexHigh == fileId2.FileIndexHigh &&
                       fileId1.FileIndexLow == fileId2.FileIndexLow;
            }

            return false;
        }

        private readonly struct FileId
        {
            public FileId(uint volumeSerialNumber, uint fileIndexHigh, uint fileIndexLow)
            {
        ...
        private static bool TryGetFileId(string path, out FileId fileId)
        {
            fileId = default;
            ...
            try
            {
                ...
                fileId = new FileId(info.VolumeSerialNumber, info.FileIndexHigh, info.FileIndexLow);
                return true;
            }
            finally
            {
            ...
            try
            {
                ...
                if (File.Exists(gmaPath))
                {
                    try
                    {
                        ...
                        // ハードリンクかどうかに関わらず削除
                        // Eハードリンクの場合でも通常のFile.Deleteで削除可能EE
                        File.Delete(gmaPath);
                    }
                    catch (Exception)
                    {
