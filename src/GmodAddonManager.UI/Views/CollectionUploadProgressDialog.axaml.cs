...
public partial class CollectionUploadProgressDialog : Window
{
    ...
    public void UpdateTotalProgress(int current, int total)
    {
        Dispatcher.UIThread.Post(() =>
        {
            ...
            if (total > 1000)
            {
                TotalProgressText.Text = L.Format("CollectionUpload.TotalProgressFormat", current, total);
    ...
    public void UpdateBatchProgress(int currentBatch, int totalBatches, int itemsInBatch, int totalItemsInBatch)
    {
        Dispatcher.UIThread.Post(() =>
        {
            BatchProgressText.Text = L.Format("CollectionUpload.BatchProgressFormat", currentBatch, totalBatches);
    ...
    public void ShowError(string error)
    {
        Dispatcher.UIThread.Post(() =>
        {
            ...
            CancelButton.Content = L.Get("Dialog.Close");
        });
    }
    ...
    private void OnCancelClick(object? sender, RoutedEventArgs e)
    {
        ...
        else
        {
            ...
            UpdateStatus(L.Get("CollectionUpload.Canceling"));
        }
    }
}
