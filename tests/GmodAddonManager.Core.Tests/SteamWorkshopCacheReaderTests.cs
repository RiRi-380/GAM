...
public sealed class SteamWorkshopCacheReaderTests
{
    ...
    [Fact]
    public void GetSubscribedAddonIdsCombinesAllCacheFiles()
    {
    ...
    [Fact]
    public void ParseAddonDetailsReadsEveryNestedItem()
    {
        const string content = """
        "AppWorkshop"
        {
    ...
    private sealed class CacheTestEnvironment : IDisposable
    {
        ...
        public string WriteSteamCache(string libraryPath, params (string Id, string Title)[] addons)
        {
            return WriteCacheFile(
                Path.Combine(libraryPath, "steamapps", "workshop", "appworkshop_4000.acf"),
                addons);
        }
        ...
        {
            return WriteCacheFile(
                Path.Combine(SteamPath, "userdata", userId, "ugc", "appworkshop_4000.acf"),
                addons);
        }
        ...
        public void WriteLibraryFolders(params string[] libraryPaths)
        {
            ...
            for (var i = 0; i < libraryPaths.Length; i++)
            {
                ...
                builder.AppendLine("    {");
                builder.AppendLine($"        \"path\" \"{libraryPaths[i].Replace(@"\", @"\\")}\"");
                builder.AppendLine("    }");
            }
